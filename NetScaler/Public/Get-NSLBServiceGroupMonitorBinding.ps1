...
function Get-NSLBServiceGroupMonitorBinding {
    ...
    process {
        foreach ($item in $Name) {
            try {
                $bindings = _InvokeNSRestApi -Session $Session -Method Get -Type servicegroup_lbmonitor_binding -Resource $item -Action Get
                return $bindings.servicegroup_lbmonitor_binding
            } catch {
                throw $_
            }
        }
