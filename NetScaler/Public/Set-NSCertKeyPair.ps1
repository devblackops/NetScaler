...
function Set-NSCertKeyPair {
    <#
    .SYNOPSIS
        Changes server certificate to NetScaler appliance.

    .DESCRIPTION
        Changes server certificate to NetScaler appliance.

    .EXAMPLE
        Set-NSCertKeyPair -CertKeyName 'myrootCA' -CertPath '/nsconfig/ssl/mycertificate.cert' -CertKeyFormat 'PEM'

        Updates a root certificate key pair named 'myrootCA' using the PEM formatted certificate 'mycertificate.cert' located on the appliance.

    .EXAMPLE
        Set-NSCertKeyPair -CertKeyName 'mywildcardcert' -CertPath '/nsconfig/ssl/mywildcard.cert' -KeyPath '/nsconfig/ssl/mywildcard.key' -CertKeyFormat 'PEM'

        Updates a certificate key pair named 'mywildardcert' using the PEM formatted certificate 'mywildcard.cert' and 'mywildcard.key' key file located on the appliance.
    ...
    process {
        if ($PSCmdlet.ShouldProcess($CertKeyName, 'Update SSL certificate and private key pair')) {
            try {
                 $params = @{
                    certkey = $CertKeyName
                    cert = $CertPath
                ...
                $response = _InvokeNSRestApi -Session $Session -Method POST -Type sslcertkey -Payload $params -Action update
            } catch {
                throw $_
            }
        }
