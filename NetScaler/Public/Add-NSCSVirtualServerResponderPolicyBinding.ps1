...
function Add-NSCSVirtualServerResponderPolicyBinding {
     <#
    .SYNOPSIS
        Adds a new content switching virtual server responder policy binding.

    .DESCRIPTION
        Adds a new content switching virtual server responder policy binding.

    .EXAMPLE
        Add-NSCSVirtualServerResponderPolicyBinding -VirtualServerName 'vserver01' -PolicyName 'pol01' -Bindpoint 'RESPONSE' -Priority '100'
    ...
    process {
        if ($PSCmdlet.ShouldProcess($VirtualServerName, 'Add Virtual Server Binding')) {
            try {
                ...
                _InvokeNSRestApi -Session $Session -Method PUT -Type csvserver_responderpolicy_binding -Payload $params

                if ($PSBoundParameters.ContainsKey('PassThru')) {
                    return Get-NSCSVirtualServerResponderPolicyBinding -Session $Session -Name $VirtualServerName
                }
            } catch {
                throw $_
            }
