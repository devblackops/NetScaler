...
function Get-NSLBSSLVirtualServerCipherGroupBinding {
    ...
    process {
        ...
        if ($PSBoundParameters.ContainsKey('VirtualServerName')) {
            $response = _InvokeNSRestApi -Session $Session -Method Get -Type sslvserver_sslcipher_binding -Action Get -Resource $VirtualServerName
        } else {
            $response = _InvokeNSRestApi -Session $Session -Method Get -Type sslvserver_sslcipher_binding -Action Get
        }

        if ($response.errorcode -ne 0) { throw $response }

        if ($response.psobject.properties | where name -eq sslvserver_sslcipher_binding) {
            return $response.sslvserver_sslcipher_binding
        }
    }
}