...
function Set-NSLBService {
    ...
    .EXAMPLE
        Set-NSLBService -Name 'sg01' -Comment 'This is a comment'

        Sets the comment for virtual server 'sg01'.

    .EXAMPLE
        Set-NSLBService -Name 'sg01' HTTPCompression = 'ON'

        Enable the HTTP compression feature for service 'sg01'.

    .EXAMPLE
        Set-NSLBService -Name 'sg01' MaxBandwithKbps 819200

        Set the maximum bandwidth for service 'sg01' to 819200 Kbps.

    .PARAMETER Session
        The NetScaler session object.

    .PARAMETER Name
        The name or names of the services to update.
    ...
    .PARAMETER Force
        Suppress confirmation when updating a service.
    ...
    process {
        foreach ($item in $Name) {
            if ($Force -or $PSCmdlet.ShouldProcess($item, 'Edit Service')) {
                $params = @{
                    name = $item
                }
